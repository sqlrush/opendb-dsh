...
docker run --rm --network host --entrypoint sh quay.io/minio/mc:latest -c "mc alias set m http://127.0.0.1:9002 opendb opendb-minio-dev >/dev/null && mc mb --ignore-existing m/dsh-test && mc mb --ignore-existing m/opendb-dsh"
