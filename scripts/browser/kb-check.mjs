...
// 向量补齐后台任务已把知识切块补到 100%（原 14%）——覆盖率应为高值，且不再有「向量缺失」健康项
check('向量覆盖率为高值(补齐后 ≥90%)', /向量覆盖率[\s\S]{0,40}?(9\d|100)%/.test(t) || /100%/.test(t), (t.match(/向量覆盖率[^\n]*/) || [''])[0]);
check('健康自检不再报「向量缺失」（已补齐）', !/向量缺失\s*[1-9]/.test(t));
