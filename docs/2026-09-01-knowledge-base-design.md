...
- **入库**：用户确认后 `kb_commit(import_id, reject=[编号], rename=[{number,src,rel,dst}])` 把未剔除候选（改名/修正过的先改）
  按 canonical 归一节点写入 `kg_nodes/kg_edges`（confidence=1.0，带来源）。
- **实体命名纪律（2026-09-05 对比实测后加）**：实体用短名词（≤12 字），条件/动作/分机号放 locator 或拆边；`kb_import` 对 >16 字的
  实体名主动提议改名。原因：图谱按实体名匹配，整句实体日后 `kg_query` 查不到——现在 `kg_query` 起点按 精确→包含→字二元组覆盖率
  三档模糊匹配并回显命中实体，但短实体名仍是根本。`knowledge_search` 命中段自动附带同文档相邻段，防条款被切块切断
  （对比实测：只命中第 2 段时模型把「BATCH_EOD 绝对不能杀」误读成「三板斧后可杀」）。实测全记录见 `2026-09-04-kb-before-after-demo.md`。
