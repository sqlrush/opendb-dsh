...
```
① 投喂        ② 分析分流(模型提议)     ③ 人审确认        ④ 入库
上传/粘贴  →  每份材料判类型 + 抽候选  →  逐条确认/纠正  →  确定性管线写三库
多文件        (向量线自动 / 图线候选)     (只审图与分流)      + 建引用台账
```

- **① 投喂**：拖拽/上传多文件（docx / pdf / txt / md / csv / 工单导出）或直接粘贴；每份可预设元数据（引擎、环境、密级），不填则进入②由模型建议。
- **② 分析分流（模型提议，不写库）**：对每份材料——
  - 判**类型**（规范 / 工单 / 故障总结 / 手册 / 拓扑说明）与**适用范围**（引擎、环境、版本、生效期）；
  - **向量线**：切块 + embed（确定性管线，不需人审，直接可入）；
  - **图线**：LLM 抽**候选三元组** `实体—关系—实体`（带来源定位、confidence），落**待确认暂存**，**不直接入图**；
  - 展示每份材料的分流预览："这份预案 → 向量 12 块 + 图 8 条候选关系"。
- **③ 人审确认**：只审两样——分流判得对不对、图候选边对不对（实体归一、关系类型、强制/建议）。**确认过的边 confidence=1.0 才可入确定性推理**；文本向量默认可入（错了影响小、可重灌）。
- **④ 入库**：确定性管线写入——向量→`knowledge_chunks`（+同步 Qdrant）、图→边表、记忆不在此列（记忆是平台自沉淀，不靠导入）。同时写**引用台账**（哪条边/块出自哪份材料哪一版）。
