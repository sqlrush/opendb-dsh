...
| `dsh-message-feedback` `dsh-client-ui-message-feedback` | 回答下方的 好的回答 / 有问题的回答（评分落 storage-domain `message_feedback`） | **禁用（2026-09-05，user「全部去掉，精简点」）**：功能可用但平台无消费方。同一行的 复制 / 在新对话中分支 是 conversation 组件自带，由 ui-harness CSS 隐藏整行 | — |
...
## 15. opendb-harness 交互纲领（user 定案 2026-08-19，产品级约束）

1. **一切交互尽量在会话里完成**。任务的创建与策略变更走对话：用户说「给我建立 SQL 审核任务，对 xx 库 top10 SQL 进行扫描并给出优化建议，每天刷新一次」→ 任务即建立；要改策略也是回到会话里说，由模型调整——**不是去任务页点按钮**。
2. **主区任务页 = 结果大盘，不是操作台**。与会话背景风格一致但内容不同：以任务结果的大盘展示为主（报告、趋势、状态），**几乎不放交互按钮，可能根本没有按钮**。大盘的展示风格最终由各任务类型的插件实现（registerTaskPanel 契约）。
3. **弹出新页面是稀缺品**。opendb-harness 极少弹页；「新建智能体」是为数不多的弹页场景（参考设置页风格）——因为它是给智能体配置管理哪些数据库、挂载哪些插件和技能、连接哪个模型的重要配置页。
4. 例外：审批/签收类人类专属决策保留显式 UI 控件（这是人对 AI 的监督闸门，不能由模型代办）。
