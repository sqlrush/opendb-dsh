# opendb-dsh Roadmap（v1，2026-08-17，待 user 评估）
...
| W3 | 数据库能力 + 采集 | `tool-pg`（只读）；`metrics`/`dictionary` seam + `metrics-timescale` + `dictionary-pg` + `collector` class + `tool-metrics`；`scheduler` |
...
- user 提供：3–5 个测试 PG 节点（含复制拓扑）；embedding 服务；MinIO/S3。

### P2 · 执行与扇出（~5–6 周）
| 周 | 主题 | 交付 |
|---|---|---|
| W1–2 | 远端执行 | `exec-ssh`、`tool-fs-search-ssh`；`tool-pg` 动作类；一次性令牌 + 白名单；preset `变更执行` |
...
## 6. 不在范围
MySQL 等其它数据库；k8s 内数据库（operator）；公有云 SaaS 化；移动端。
