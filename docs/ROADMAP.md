...
| 资源 › 模型用量（2026-08-31，user 通过 `docs/prototypes/usage-r2.html` 后开发）✅ | **ui-cluster** 加第二个面板（摘要 6 卡 · 逐日堆叠趋势 + 独立调用次数带 · 来源/模型构成 · 单次规模 · Top 会话可打开）· platform-status 扩 `usage` 端点（只读 `assistant/message.usage`，四字段全由模型 API 原样返回、不估算不折算费用）· migration 020 两条部分索引把首屏 5s 压到 0.1s · platform-status 旧资源大盘 client 面板下线 |
| 平台规则目录 R1（2026-09-01，user 通过 `docs/prototypes/rules-r1.html` 后开发）✅ | **task-rules** 面板重做（搜索/筛选 · 彩色级别阶梯 · ⚙ 可调阈值与当前生效值 · 近 30 天命中 N/M 轨道条 · 展开看判据/改动理由/最近命中原文）· 新增 `/opendb-rules` 通道（`stats.ts` 只读采集存档 + 阈值服务 `list()`）· 目录补齐容量插件 10 条 CAP_* 与健康 OS_LOAD_HIGH/OS_IOWAIT_HIGH · 新增「阈值 spec 的规则码必须在目录里」单测（61 项对账） |
| 知识库导入工具 P2 + 强类型图 P3（2026-09-02）✅ | 导入向导（向量线服务端确定性入库 + 图线模型 `kb_extract` 抽候选 → 人审默认纳入 → commit 进 `kg_nodes/kg_edges`,confidence=1.0）· migration 021（imports/staging/kg 四表）· `knowledge-pg` createImport/stageEdges/commitImport + 向量补齐后台任务 · `tool-knowledge` kb_import/kb_extract · e2e `kb-import-check.mjs` 10/10 |
| 知识库 › 知识库大盘 P1（2026-09-01，user 通过 `docs/prototypes/knowledge-r1.html` 后开发）✅ | **ui-kb**（client-only 面板：记忆/向量/图三库全貌 + 健康自检）· `knowledge-pg` 加 `dashboard()` 只读聚合 · `ui-knowledge` 加 `dashboard` 端点 · `ui-harness` 侧栏「知识库」一级目录（`registerKnowledgePanel` 按 key）· 设计见 `docs/2026-09-01-knowledge-base-design.md`；P2 导入工具 / P3 强类型图+报告接入待做 |
