...
- **知识库前后对比实测暴露的两处检索缺口（2026-09-04/05 演示，记录见 `docs/2026-09-04-kb-before-after-demo.md`）**：
  ...
  短词「核心库锁等待」查不到。改为三档匹配：精确 → 任一方向包含 → 查询词**字二元组覆盖率**（纯函数 `entityMatchScore`，
  单测 3 组；不用 pg_trgm——本库 locale 下 trigram 对中文打分为 0，实测），结果回显 `matched` 让模型知道按什么实体在查，
  起点第一跳同时取入边与出边。配套：`kb_import` 引导实体用短名词（≤12 字）、实体名 >16 字时主动向用户提议改名；
  `kb_commit` 新增 `rename=[{number,src,rel,dst}]` 按编号改名/修正后入图。
  ② 规范被切块切在条款/表格中间，用户复测只命中第 2 段，模型把「BATCH_EOD 绝对不能杀」误读成「三板斧后可杀」。
  `knowledge_search` 命中某段时自动附带同文档相邻段（seq±1，上限 6），小文档等于整篇带上。

### Changed
- **知识库导入改为「会话式」，去掉导入向导页面（user 2026-09-03 定，遵纲领 §15：一切交互尽量在会话完成）**：
  用户在会话里说「把 xxx 导入知识库」→ 智能体调 `kb_import` 解析材料（向量线确定性切块入库 + 关系候选**暂存、不入图**）→
  **主动用 `ask_user_question` 逐条向用户提问**（材料分类、适用引擎/环境、置信度偏低的关系是否保留、实体是否需合并/改名）→
  用户确认/指正后智能体调 `kb_commit(import_id, reject=[编号])`，未剔除的候选写进强类型图（confidence=1.0），**结果在会话里展示**。
  移除 `ui-kb` 的导入向导面板与 `ui-knowledge` 的 imports/staging RPC 端点、`kb_extract` 工具；知识库大盘保留（只读看全貌）。
  实测：一句「把 openGauss 变更规范导入」→ 智能体暂存 4 条候选（核心库锁等待→causes→交易超时 等）、kg 保持 0、随即发起结构化提问。
