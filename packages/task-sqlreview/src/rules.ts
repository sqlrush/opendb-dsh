...
export async function ruleTbl001(q: QueryFn, slowTexts: string[]): Promise<RuleFinding[]> {
...
/** TBL002 大表无任何索引 */
export async function ruleTbl002(q: QueryFn): Promise<RuleFinding[]> {
  const rows = (await q(`SELECT c.relname AS t, n.nspname AS s, c.reltuples::bigint AS n
FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE c.relkind = 'r' AND n.nspname NOT IN ('pg_catalog','information_schema','dbe_perf','dbe_pldeveloper','db4ai','cstore','pg_toast')
  AND c.reltuples > 100000 AND NOT EXISTS (SELECT 1 FROM pg_index i WHERE i.indrelid = c.oid)
...
/** IDX002 索引列数过多（>5） */
export async function ruleIdx002(q: QueryFn): Promise<RuleFinding[]> {
...
WHERE i.indnatts > 5 AND ns.nspname NOT IN ('pg_catalog','information_schema','dbe_perf','dbe_pldeveloper','db4ai') LIMIT 6`, 6)).rows;
  return rows.map((r) => ({
...
export async function ruleCol001(q: QueryFn): Promise<RuleFinding[]> {
  const rows = (await q(`SELECT c.relname AS t, a.attname AS col, a.atttypmod - 4 AS len
FROM pg_attribute a JOIN pg_class c ON c.oid = a.attrelid JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE a.atttypid IN ('varchar'::regtype) AND a.atttypmod - 4 > 4000 AND c.relkind = 'r' AND a.attnum > 0
...
/** 目录类规则编排：逐条独立降级 */
export async function runCatalogRules(q: QueryFn, slowTexts: string[], notes: string[]): Promise<RuleFinding[]> {
  const jobs: { name: string; run: () => Promise<RuleFinding[]> }[] = [
    { name: 'TBL001', run: () => ruleTbl001(q, slowTexts) },
    { name: 'TBL002', run: () => ruleTbl002(q) },
    { name: 'IDX001', run: () => ruleIdx001(q) },
    { name: 'IDX002', run: () => ruleIdx002(q) },
    { name: 'IDX003/004', run: () => ruleIdx003And004(q) },
    { name: 'IDX005', run: () => ruleIdx005(q) },
    { name: 'COL001', run: () => ruleCol001(q) },
  ];
  const out: RuleFinding[] = [];
  for (const j of jobs) {
    try {
