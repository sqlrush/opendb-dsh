...
function Donut({ counts }: { counts: any }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 18, alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, color: T.sub }}>
        {parts.map((p) => (
...
function RuleBars({ findings }: { findings: any[] }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 7, fontSize: 12, flex: 1, minWidth: 240 }}>
      <div style={{ fontSize: 12, color: T.dim }}>规则命中排行</div>
      {rows.map(([rule, v]) => (
        <div key={rule} style={{ display: 'grid', gridTemplateColumns: '64px 1fr 24px', gap: 8, alignItems: 'center' }}>
          <span style={{ font: `600 10.5px ${mono}`, background: T.fill, border: `1px solid ${T.line}`, borderRadius: 5, padding: '1px 6px', color: T.sub, textAlign: 'center' }}>{rule}</span>
...
function CostBars({ orig, next }: { orig: number; next: number }) {
  ...
  const row = (label: string, v: number, color: string, txt: string) => (
    <div style={{ display: 'grid', gridTemplateColumns: '56px 1fr 130px', gap: 10, alignItems: 'center', fontSize: 12 }}>
...
function SqlCard({ s }: { s: any }) {
  ...
  return (
    <div style={{ ...card, marginBottom: 12 }}>
      ...
      {(s.plan ?? []).length > 0 ? (
        <>
          <div style={{ fontSize: 12, color: T.dim, margin: '10px 0 4px', fontWeight: 500 }}>原执行计划 <span style={{ fontWeight: 400 }}>· 优化点直接标注在计划行上</span></div>
          <PlanTree plan={(s.plan ?? []).map(String)} cost={orig} />
        </>
      ) : (s.planNotes ?? []).length > 0 ? (
        <>
          <div style={{ fontSize: 12, color: T.dim, margin: '10px 0 4px', fontWeight: 500 }}>原计划优化点（脚本标注）{orig > 0 ? <span style={{ fontWeight: 400 }}>· 总 cost {orig.toLocaleString()}</span> : null}</div>
      ...
      {String(s.detail ?? '') !== '' ? <div style={{ fontSize: 12.5, color: T.sub, marginTop: 8 }}>{String(s.detail)}</div> : null}
