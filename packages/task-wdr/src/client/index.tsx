...
function Timeline({ snaps }: { snaps: any[] }) {
  ...
  return (
    <div style={{ ...card, overflowX: 'auto' }}>
      ...
      <div style={{ fontSize: 11, color: T.dim }}>快照每小时自动产生（enable_wdr_snapshot）· 换窗口在会话里说一句（"看下 14 点到 15 点"）</div>
    </div>
  );
}
...
function LoadStack({ dbTime }: { dbTime: any }) {
  ...
  if (total <= 0 || classes.length === 0) return <div style={{ fontSize: 12, color: T.dim }}>窗口内 DB Time 近零——数据库基本空闲。</div>;
  return (
    <div>
      ...
      <div style={{ display: 'flex', height: 26, borderRadius: 6, overflow: 'hidden', maxWidth: 680, border: `1px solid ${T.line}` }}>
        {classes.map((c: any, i: number) => (
          <i key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10.5, color: '#fff', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', fontStyle: 'normal', width: `${Math.max(2, Number(c.share) * 100)}%`, background: CLASS_COLORS[String(c.name)] ?? '#9aa3b5' }}>
            {Number(c.share) >= 0.08 ? `${String(c.name)} ${(Number(c.share) * 100).toFixed(0)}%` : ''}
          </i>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 14, fontSize: 11.5, color: T.sub, marginTop: 6, flexWrap: 'wrap' }}>
        {classes.map((c: any, i: number) => (
  ...
  return (
    <div style={{ fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Hiragino Sans GB","Microsoft YaHei",sans-serif', color: T.ink, lineHeight: 1.6 }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
        <span style={{ fontSize: 16, fontWeight: 600, color: sev(worst).c }}>窗口态势：{sev(worst).cn}</span>
        ...
        <span style={{ display: 'inline-flex', gap: 6, fontSize: 12, background: T.sev.ok.soft, color: T.sev.ok.c, borderRadius: 6, padding: '3px 10px', fontWeight: 500 }}>✓ 已锚定 · 七维 delta 由脚本产出 · 归因不可改</span>
        <span style={{ fontSize: 12, color: T.dim }}>📄 只读展示 · 不动快照配置</span>
      </div>
        ...
        <table style={{ borderCollapse: 'collapse', width: '100%' }}>
          ...
          <tbody>
            {((data.topSql ?? []) as any[]).map((s: any, i: number) => {
              ...
              return (
                <tr key={i}>
                  <td style={{ ...td, fontFamily: mono }}>{String(s.sqlId)}<div style={{ color: T.dim, fontSize: 11, maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{String(s.text)}</div></td>
                  <td style={td}><span style={{ fontSize: 11, fontWeight: 600, borderRadius: 5, padding: '1px 7px', whiteSpace: 'nowrap', color: b.c, background: b.bg }}>{b.t}</span></td>
      ...
      {(data.loadProfile ?? []).length > 0 ? (
        <>
          <H2>Load Profile <span style={{ fontSize: 12, color: T.dim, fontWeight: 400 }}>窗口 delta（µs）</span></H2>
          <div style={{ ...card, padding: 0, overflowX: 'auto' }}>
            <table style={{ borderCollapse: 'collapse', width: '100%' }}>
      ...
      {(data.waits ?? []).length > 0 ? (
        <>
          <H2>等待事件 <span style={{ fontSize: 12, color: T.dim, fontWeight: 400 }}>窗口 delta · 已剔除空闲 STATUS 类</span></H2>
          <div style={card}>
            {((data.waits ?? []) as any[]).slice(0, 8).map((w: any, i: number) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '90px 1fr 70px', gap: 10, alignItems: 'center', fontSize: 12, marginBottom: 5 }}>
      ...
      <H2>发现 <span style={{ fontSize: 12, color: T.dim, fontWeight: 400 }}>七维阈值判定 · 级别不可被解读下调</span></H2>
      {findings.filter((f) => String(f.level) !== 'ok').length === 0
        ? <div style={{ fontSize: 13, color: T.sev.ok.c }}>✓ 窗口内各维在阈值内。</div>
        : findings.map((f: any, i: number) => (
          <div key={i} style={{ ...card, borderLeft: `3px solid ${sev(String(f.level)).c}`, marginBottom: 8 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <Dot level={String(f.level)} /><b style={{ fontSize: 13 }}>{String(f.detail)}</b>
              <span style={{ font: `600 10.5px ${mono}`, background: T.fill, border: `1px solid ${T.line}`, borderRadius: 5, padding: '1px 6px', color: T.sub }}>{String(f.code)}</span>
              {String(f.value ?? '') !== '' ? <span style={{ marginLeft: 'auto', fontSize: 11, color: T.dim }}>实测 <b style={{ color: sev(String(f.level)).c }}>{String(f.value)}</b> · 阈值 {String(f.threshold)}</span> : null}
            </div>
      ...
      {String(data.nativeReport ?? '') !== '' ? (
        <div style={{ display: 'flex', gap: 10, background: T.fill, borderRadius: 10, padding: '12px 16px', fontSize: 12, color: T.sub, marginTop: 16 }}>📄 {String(data.nativeReport)}</div>
      ) : null}
      {(data.collectionNotes ?? []).length > 0 ? (
        <div style={{ fontSize: 12, color: T.dim, border: `1px dashed ${T.line}`, borderRadius: 8, padding: '8px 14px', marginTop: 10 }}>
