 ...
 * 导入不走页面：在会话里说「把 xxx 导入知识库」，模型解析→逐条提问确认→入库（kb_import/kb_commit 工具）。
 */
...
function Dashboard({ call }: { call: (endpoint: string, payload?: unknown) => Promise<any> }) {
  ...
  if ((d.graph.pendingReview ?? 0) > 0) issues.push({ lv: 'notice', t: `图候选待确认 ${fmtInt(d.graph.pendingReview)} 条`, d: '有一次会话导入正在进行——模型抽了关系候选、等你在会话里确认后入图', act: '回会话确认' });
  if (d.graph.typed === false && d.graph.edges > 0 && (d.graph.kgEdges ?? 0) === 0) issues.push({ lv: 'notice', t: `图边尚未定型 ${fmtInt(d.graph.edges)} 条`, d: '现为记忆实体共现边（无类型/来源/版本）；导入客户资料确认入图后成为强类型边', act: '会话导入' });
  if (d.vector.docs <= 2 && (d.graph.kgEdges ?? 0) === 0) issues.push({ lv: 'crit', t: '客户专属知识为空', d: '尚未导入客户规范 / 工单 / 故障总结——在会话里说「把 xxx 导入知识库」', act: '会话导入' });
  ...
  return (
    <div style={{ fontFamily: FONT, color: T.ink, lineHeight: 1.6 }}>
      ...
      <div style={{ fontSize: 13, color: T.dim, border: `1px dashed ${T.line}`, borderRadius: 8, padding: '10px 16px', marginTop: 18, lineHeight: 1.8 }}>
        📥 <b>知识从哪来</b>：记忆由平台任务报告自动沉淀；向量与图由<b>会话导入</b>——在会话里说「把 xxx 导入知识库」，模型解析材料、逐条向你提问确认后写入（拓扑/依赖/变更史另由采集器直写图，不经导入）。<br />
...
export function apply(ctx: any): void {
  ...
  // 只注册只读大盘；导入走会话（模型解析→逐条提问确认→入库），不做页面（纲领 §15）。
