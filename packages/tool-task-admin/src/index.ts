...
function defineTaskCreateTool(deps: Deps) {
  return defineTool({
    ...
    parameters: {
      ...
      config: { type: 'object', additionalProperties: true, description: '任务类型的配置对象。目标节点各类型统一支持 node（单个）或 nodes（数组）：health={node|nodes,dims,focus}、sqlreview（Top SQL 报表）={node,dimensions,topN,sqls,focus}——dimensions 是榜单维度数组，用户说"按执行次数和耗时分别 Top5"就填 ["calls","elapsed"]、topN=5（可选 elapsed=总耗时/calls=执行次数/avg=平均耗时/cpu/io/blocks=逻辑读/dbtime/spill=下盘/rows=返回行数），**这类需求必须用 sqlreview 类型，不要退化成 prompt 定时对话**；sqls 只放用户自己贴的、榜单之外的 SQL，榜单里会出现的语句不要再塞进 sqls、wdr={node,beginSnap,endSnap,topN}、ddl={node,hours}、rules={plugin}。**用户点名了具体节点就必须填**，不填 = 该智能体绑定的全部节点（可能是几百个）。' },
